...
if v_count <> 4 then raise_application_error(-20040,
...
v_count := 0;
--replica local
delete from tipo_cuenta_r1 where tipo_cuenta_id = :old.tipo_cuenta_id; v_count := v_count + sql%rowcount;
--replica 2
delete from tipo_cuenta_r2 where tipo_cuenta_id = :old.tipo_cuenta_id; v_count := v_count + sql%rowcount;
--replica 3
delete from tipo_cuenta_r3 where tipo_cuenta_id = :old.tipo_cuenta_id; v_count := v_count + sql%rowcount;
--replica 4
delete from tipo_cuenta_r4 where tipo_cuenta_id = :old.tipo_cuenta_id; v_count := v_count + sql%rowcount;
if v_count <> 4 then raise_application_error(-20040,
